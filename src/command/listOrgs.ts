...
import { OpenOperation } from './helper/openFileInOrg'; // Enum

export enum OrgType { Scratch = 'Scratch Orgs', DevHub = 'Dev Hubs', ScratchDev = 'Scratch Orgs and Dev Hubs' }
export enum Operation { Open, Delete, Validate, OpenFile, OpenHelp }
 ...
 * @param operation - Operation to perform on org list - `Open | Validate | Delete | OpenFile | OpenHelp`.
...
export function listAllOrgs(type: OrgType, operation: Operation, context: vscode.ExtensionContext, filePath?: string) {
	...
	if (operation === Operation.Open || operation === Operation.Delete || operation === Operation.Validate || operation === Operation.OpenFile) {
			...
			window.showQuickPick(userAliasAssociative, { placeHolder: 'Select an org to open', matchOnDescription: true }).then(returnValue => {
				if (typeof returnValue !== 'undefined') { // Otherwise error "returnValue is possibly 'undefined'"
					...
					if (operation === Operation.Delete) {
						...
						openFileInOrg(orgName, filePath, OpenOperation.OpenFile, context);
					} else {
	...
	} else if (operation === Operation.OpenHelp && filePath !== undefined) {
		vscode.window.setStatusBarMessage('Opening help...', 5000);
		openFileInOrg('', filePath, OpenOperation.OpenHelp, context);
	}
}