...
export function openFileInOrg(
	...
	const componentLinks = require(context.extensionPath +
		'/src/config/componentLinks.json');

	const fileNameExt = utils.getFileName(filePath);
	console.log('File Name (full name): ' + fileNameExt);
	let file: string;
	if (fileNameExt.includes('.') && fileNameExt.split('.').length <= 3) {
		// Opportunity.workflow-meta.xml
		file = fileNameExt.split('.')[0];
	} else if (fileNameExt.includes('.') && fileNameExt.split('.').length > 3) {
		// Account.actionName.quickAction-meta.xml
	...
	let type: string;
	if (filePath.includes('-') && filePath.split('.').length <= 3) {
		// X.permissionset-meta.xml
		type = filePath
			.split('/')
			.slice(-1)[0]
			.split('.')[1]
			.split('-')[0]; // utils.getFolderName(filePath); // Easier to retrieve type from extension than folder, as we have files in sub-subfolders
	} else if (filePath.split('.').length > 3) {
		// path/Account.ActionName.quickAction-meta.xml
		type = filePath
			.split('/')
			.slice(-1)[0]
			.split('.')[2]
	...
	if (operation === OpenOperation.OpenFile) {
		if (componentLinks[type] === undefined) {
			vscode.window
				.showErrorMessage(
					'Cannot open this file, please open it manually.',
					'OK',
					'Open Org'
				)
				.then((button) => {
			...
			if (componentLinks[type].exceptions !== undefined) {
				...
				for (var i = 0; i < exceptions.length; i++) {
					if (file in exceptions[i]) {
						console.log(
							'Replaced ' + file + ' with ' + exceptions[i][file]
						);
			...
			if (componentLinks[type].object === undefined) {
				// Simple link, does not require getting Id
				console.log(
					'Opening: ' +
						componentLinks[type].link.replace('{%s}', file)
				);
				...
				vscode.window.withProgress(
					...
					async (progress, token) => {
						await new Promise((resolve) => {
							...
							const api =
								componentLinks[type].api !== undefined &&
								componentLinks[type].api === 'tooling'
									? ' -t'
									: '';
							console.log('Query: ' + query);
							cp.exec(
								'sfdx force:data:soql:query --json -q "' +
									query +
									'"' +
									api +
									' -u "' +
								...
								function(error: any, output: any) {
									...
									if (
										...
										vscode.window.showErrorMessage(
											'Could not open file. Either it does not exist on your org or it cannot be opened directly.',
											'OK'
										);
										resolve();
									}
		...
		if (componentLinks[type] === undefined) {
			vscode.window
				.showErrorMessage(
					'Cannot determine help article for this file, please navigate yourself.',
					'OK',
					'Open Help'
				)
				.then((button) => {
			...
			vscode.env.openExternal(
				vscode.Uri.parse(
					'https://developer.salesforce.com/docs/atlas.en-us.api_meta.meta/api_meta/' +
						componentLinks[type].metadataHelp +
						'.htm'
				)
			);
		}
	...
	function openFile(link: string, file: string, path?: string) {
		vscode.window.withProgress(
			...
			async () => {
				if (path !== undefined && path.includes('flexipage')) {
					// Flexipages use a different command
					console.log(
	...
	function generateQuery(componentLinks: any): string {
		...
		if (componentLinks[type].queryAddition === undefined) {
			// Simple query
			queryWhere =
				' WHERE ' + componentLinks[type].field + " = '" + file + "'";
		} else if (type === 'layout') {
			// Account-Account Layout.layout-meta.xml, split at -
			queryWhere =
				' WHERE ' +
				componentLinks[type].queryAddition
					.replace('{%obj}', file.split(/-(.+)/)[0])
					.replace('{%name}', file.split(/-(.+)/)[1]);
		} else if (type === 'quickAction') {
			// Account.MyQuick.xml, split at .
			queryWhere =
				' WHERE ' +
				componentLinks[type].queryAddition
					.replace('{%obj}', file.split('.')[0])
			...
			// Name.xml, but we require Object and Id. Extract Name, get Object from path.
			queryWhere =
				' WHERE ' +
				componentLinks[type].queryAddition
					.replace('{%obj}', parentParentFolder)
					.replace('{%name}', file.split('.')[0].replace('__c', '')); // CustomFields need to be changed from Field__c to Field. For other types it does not matter..
		} else {
			// Name.flow-meta.xml
			queryWhere =
				' WHERE ' +
				componentLinks[type].queryAddition.replace('{%name}', file);
		}
	...
	function generateLink(componentLinks: any, response: any): string {
		...
		if (
			...
			// Standard/CustomFields, ListViews, RecordTypes and ValidationRules require object and name in URL as they are in subfolders.
			...
			// Replace object key prefix
			link = componentLinks[type].link.replace(
				'{%id}',
				response.result.records[0].DurableId.replace(
					'0KD',
					'01r'
				).substring(0, 15)
			);
