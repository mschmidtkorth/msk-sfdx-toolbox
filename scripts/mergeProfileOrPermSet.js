...
async function run(branch, filePath) {
  if (!fs.existsSync(filePath)) {
    throw new Error(`ERROR: ${filePath} is not a valid file`);
  }
...
if (process.argv.length < 4) {
  throw new Error('ERROR: You have to specify a branch name and a file');
}
